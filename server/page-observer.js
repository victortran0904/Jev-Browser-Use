...
(() => {
  ...
  function capture() {
    ...
    const secrets=Array.from(document.querySelectorAll('input,textarea,[contenteditable=true]')).filter(sensitive).map(el=>String('value' in el?el.value:el.textContent||'')).filter(v=>v.length>=3);
    ...
    if(cached && [...entries.values()].some(e=>!e.el.isConnected || identity(e.el)!==e.signature || !visible(e.el))) dirty=true;
    ...
    for (const el of full ? document.querySelectorAll(selector) : []) {
      ...
      const label=`${role(el)} ${JSON.stringify(name(el))}`;
      const ref=refFor(el), signature=identity(el);
      entries.set(ref,{el,signature});
    ...
    if (el instanceof HTMLElement && el!==document.body) {
      const ref=refFor(el), signature=identity(el);
      entries.set(ref,{el,signature});
      focusedField={ref,signature,label:name(el),placeholder:el.getAttribute('placeholder')||'',value:sensitive(el)?'':String('value' in el?el.value:el.textContent||'').slice(0,300),isText:isText(el)};
    }
    ...
    return {documentId,candidates,focusedField,pageText,url:redact(location.href),title:redact(document.title),metrics:{domExtractMs:performance.now()-started,mode:full?'full':'focused'}};
  }
