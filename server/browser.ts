...
export interface BrowserBoundaryOptions { settleTimeoutMs?: number; }
...
export function createBrowserBoundary(command: CommandRunner = defaultCommandRunner, _options: BrowserBoundaryOptions = {}): BrowserBoundary {
  ...
  return {
    ...
    async act(runId,action,observation) {
      ...
      const target=action.kind==="type_text" ? observation.focusedField : observation.candidates.find(c=>c.ref===action.target);
      const request={ref:target?.ref,signature:target?.signature,documentId:observation.documentId,text:action.kind==="type_text",focused:action.kind==="type_text"};
      ...
      const scripts:Record<string,string>={
        ...
        press_enter:`await p.keyboard.press("Enter"); return "pressed Enter";`,
    ...
    async close(runId) {
      ...
      if(!sessions.has(`jev-${runId}`)) return;
      try {
        await command(["execute","--json","--session",`jev-${runId}`,`if(state.__jevObserver) { await state.__jevObserver.evaluate(api=>api.release()).catch(()=>{}); await state.__jevObserver.dispose().catch(()=>{}); } for(const owned of state.__jevPages||[]) {if(owned!==page && !owned.isClosed()) await owned.close().catch(()=>{});} return true;`]);
      } finally {
        await command(["session","delete",`jev-${runId}`]); sessions.delete(`jev-${runId}`);
      }
    },
  };
}
