...
export async function createRuntime(route, { relay = false } = {}) {
  ...
  try {
    ...
    if (relay) {
      ...
      children.push(spawn(chromium.executablePath(), [`--user-data-dir=${path.join(temp, 'profile')}`, `--disable-extensions-except=${extension}`, `--load-extension=${extension}`, '--no-first-run', '--disable-popup-blocking', '--no-default-browser-check', '--no-sandbox', '--disable-dev-shm-usage', '--ignore-certificate-errors', ...(!process.env.DISPLAY ? ['--headless=new'] : []), 'about:blank'], { env: childEnv(), stdio: 'ignore' }));
      let ready = false;
      for (let i = 0; i < 180; i++) {
    ...
    browser = await chromium.launch({ executablePath: chromium.executablePath(), headless: true, env: childEnv(), args: ['--no-sandbox'] });
