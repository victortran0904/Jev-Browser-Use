...
for (const file of (await readdir(new URL('./cases/', import.meta.url))).filter(n => n.endsWith('.mjs')).sort()) {
  const { default: register } = await import(new URL('./cases/' + file, import.meta.url));
  register({ cases, routes, html, boundary, runtime, click, choose, observeUntil, assert });
}
