...
function classify(error) {
  ...
  return /401|invalid.api.key|authentication/i.test(message)?'authentication':/403|forbidden/i.test(message)?'permission':/429|quota/i.test(message)?'rate-limit':/timeout|timed.out|aborted/i.test(message)?'timeout':/503|unavailable|overload/i.test(message)?'provider-unavailable':/budget/i.test(message)?'request-budget':/stale/i.test(message)?'stale-state':/step limit/i.test(message)?'step-limit':/missing.*secret/i.test(message)?'missing-secrets':'assertion-or-runtime-failure';
}
...
async function check(id,fn) {
  ...
  catch(error){report.tests.push({id,status:'fail',durationMs:Math.round(performance.now()-start),error:classify(error)});console.log(`FAIL ${id}: ${classify(error)}`);}
}
try {
  ...
  async function liveRun(scenario,publicFlight=false) {
    ...
    const controller=createRunController({browser:safeBoundary,planner:makePlanner(),writer:createWriter(),enableScreenshots:false});
    ...
    if(publicFlight) {
      // Conservative: all facts must occur in the same visible line. A landing
      // page teaser or model's done declaration is not verified flight evidence.
    ...
    assert(final?.snapshot.includes(scenario.proof),'Missing visible completion evidence');
    assert.equal(run.status,'complete');
    return {stepCount:run.stepCount,timings:run.timings};
  }
