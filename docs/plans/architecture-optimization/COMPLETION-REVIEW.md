...
Historical note: this review originally left changed-page-only recovery from an uncertain decision unimplemented after a blocked command. The later `ULTRAFAST-REFERENCE-REVIEW.md` iteration re-ran that RED case through the active public controller, implemented it test-first, and preserved the original failing case as `archive/historical-uncertain-state-recovery-red.test.ts.txt`. This paragraph records the earlier state and is superseded for current runtime behavior.

Historical note: at the end of this review, source still matched runtime `29d21c9`. The later `ULTRAFAST-REFERENCE-REVIEW.md` iteration intentionally changes runtime behavior with new RED/GREEN evidence, so this older statement is no longer the current head description. No real-flight pass should be inferred from this historical checkpoint.
