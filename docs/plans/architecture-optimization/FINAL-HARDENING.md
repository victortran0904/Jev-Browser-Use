...
## Reconciliation with `4f95eec`

The PR branch advanced during isolated review. Both histories are preserved in a normal merge. Its shared `page.controls` model-state fix and all its tests are retained; item criteria remain compact references so each independent Jev question can see control semantics without duplicating labels.

For overlapping recovery changes, the merged implementation uses the private structured pre-dispatch result and shared error module, re-exports `StaleObservationError` from the existing browser module for compatibility, and preserves the public `state_refresh` event. The stricter remote cap of **two refreshes** applies to both stale observations and invalid model targets; the third rejection stops, still within the unchanged twelve-decision ceiling. The earlier local twelve-step preservation assertion was adapted to this stricter merged behavior, not relaxed to hide a failure.
